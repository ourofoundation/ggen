...
class StructureDatabase:
    ...
    def _init_database(self) -> None:
        ...
        conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_hull_structure "
            "ON hull_entries(structure_id, computed_at DESC)"
        )
    ...
    def get_system_statistics(
        self, chemical_systems: List[str], e_above_hull_cutoff: float = 0.05
    ) -> Dict[str, Any]:
        """Summarize everything the database knows about one or more systems.

        Counts cover each system and all of its subsystems, since a Bi-Fe-S hull
        is shaped by the Bi, Fe, S, Bi-Fe, … structures as well. Structures are
        counted once even when the systems share subsystems, while hull and run
        counts are summed across them.
        """
        chemsys_list = [self.normalize_chemsys(system) for system in chemical_systems]
        subsystems = sorted(
            {
                subsystem
                for chemsys in chemsys_list
                for subsystem in self.get_all_subsystems(chemsys.split("-"))
            }
        )
        placeholders = ",".join("?" * len(subsystems))

        totals = self.conn.execute(
            f"""
            SELECT COUNT(*) AS structures,
                   COUNT(DISTINCT formula) AS formulas,
                   COUNT(DISTINCT chemsys) AS subsystems
            FROM structures
            WHERE is_valid = 1 AND chemsys IN ({placeholders})
            """,
            subsystems,
        ).fetchone()
        ...
        # Terminal elements are on the hull by construction, so the hull counts
        # only cover compounds. Each hull is per system, hence the summation.
        system_placeholders = ",".join("?" * len(chemsys_list))
        hull = self.conn.execute(
            f"""
            SELECT SUM(h.is_on_hull) AS on_hull,
                   SUM(CASE WHEN h.e_above_hull <= ? THEN 1 ELSE 0 END) AS near_hull
            FROM hull_entries h
            JOIN structures s ON s.id = h.structure_id
            WHERE h.chemsys IN ({system_placeholders}) AND s.chemsys LIKE '%-%'
            """,
            [e_above_hull_cutoff, *chemsys_list],
        ).fetchone()

        runs = self.conn.execute(
            f"""
            SELECT COUNT(*) AS total, MAX(completed_at) AS last_completed_at
            FROM runs WHERE chemical_system IN ({system_placeholders})
            """,
            chemsys_list,
        ).fetchone()
        ...
        return {
            ...
            "unique_formulas": totals["formulas"],
            "subsystems_covered": totals["subsystems"],
            "by_source": by_source,
            "on_hull": hull["on_hull"] or 0,
            "near_hull": hull["near_hull"] or 0,
            "e_above_hull_cutoff": e_above_hull_cutoff,
            "total_runs": runs["total"],
            "last_run_at": runs["last_completed_at"],
        }

    def get_run_statistics(self, run_id: str) -> Dict[str, Any]:
        """Count the structures a single exploration run contributed."""
        row = self.conn.execute(
            """
            SELECT COUNT(*) AS structures, COUNT(DISTINCT s.formula) AS formulas
            FROM run_structures rs
            JOIN structures s ON s.id = rs.structure_id
            WHERE rs.run_id = ? AND s.is_valid = 1
            """,
            (run_id,),
        ).fetchone()
        return {
            "structures": row["structures"],
            "unique_formulas": row["formulas"],
        }

    def get_known_polymorphs(self, chemical_system: str) -> Dict[str, Set[int]]:
        """Map reduced formula to the space groups known from reference data.

        Everything not generated by ggen (Materials Project, Alexandria, …) is
        prior art, which is what makes novelty checkable: a generated structure
        whose composition is missing here is new, and one whose space group is
        missing is a new polymorph. Keys are reduced formulas so a generated
        Co2Fe2 cell is matched against the published CoFe.
        """
        chemsys = self.normalize_chemsys(chemical_system)
        subsystems = self.get_all_subsystems(chemsys.split("-"))
        placeholders = ",".join("?" * len(subsystems))

        rows = self.conn.execute(
            f"""
            SELECT formula, space_group_number FROM structures
            WHERE is_valid = 1 AND source != 'ggen' AND chemsys IN ({placeholders})
            """,
            subsystems,
        ).fetchall()
        ...
        for row in rows:
            ...
            known.setdefault(reduced, set()).add(row["space_group_number"])
        return known

    @staticmethod
    def classify_novelty(
        structure: StoredStructure, known_polymorphs: Dict[str, Set[int]]
    ) -> str:
        """Classify a structure against the reference data from a system.

        Returns "reference" for imported structures, and for generated ones
        "new composition", "new polymorph", or "known", using the map returned
        by :meth:`get_known_polymorphs`.
        """
        if (structure.source or "ggen") != "ggen":
            return "reference"
        reduced = Composition(structure.formula).reduced_formula
        space_groups = known_polymorphs.get(reduced)
        if space_groups is None:
            return "new composition"
        if structure.space_group_number not in space_groups:
