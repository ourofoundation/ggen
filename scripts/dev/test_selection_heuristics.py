...
def analyze_correlations(candidates: List[CandidateData]) -> dict:
    ...
    ranking = {
        ...
        "true_best_rank_by_initial": int(np.where(np.argsort(initial_e) == best_final_idx)[0][0]) + 1,
        "true_best_rank_by_mini_delta": int(np.where(np.argsort(-mini_delta) == best_final_idx)[0][0]) + 1,
    }
...
def print_results(analysis: dict, candidates: List[CandidateData]):
    ...
    print(f"  Final E range:      {stats['final_energy_range'][0]:.2f} to {stats['final_energy_range'][1]:.2f} eV")
    ...
    print(f"  True best's rank by initial_energy: {rank['true_best_rank_by_initial']} / {stats['num_candidates']}")
    print(f"  True best's rank by mini_delta:     {rank['true_best_rank_by_mini_delta']} / {stats['num_candidates']}")
    ...
    print(f"  {'Rank':<5} {'SG':<5} {'Init E':<12} {'Force RMS':<12} {'Mini Δ':<12} {'Final E':<12}")
    for i, c in enumerate(sorted_by_final[:5]):
        print(f"  {i+1:<5} {c.space_group:<5} {c.initial_energy:<12.3f} {c.initial_force_rms:<12.3f} {c.mini_relax_delta:<12.3f} {c.final_energy:<12.3f}")
    ...
    for i, c in enumerate(sorted_by_initial[:5]):
        ...
        print(f"  {i+1:<5} {c.space_group:<5} {c.initial_energy:<12.3f} {c.initial_force_rms:<12.3f} {c.mini_relax_delta:<12.3f} {c.final_energy:<12.3f} (final rank: {final_rank})")
...
def main():
    ...
    parser.add_argument("--num-candidates", type=int, default=50, help="Number of candidates to generate")
