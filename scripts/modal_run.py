...
Usage:
    ...
    modal run scripts/modal_run.py --system Fe-Co-Bi --max-atoms 16
...
volume = modal.Volume.from_name("ggen", create_if_missing=True)

# Image with all dependencies for GPU-accelerated ggen
# Include local ggen package via add_local_dir (Modal 1.0 API)
image = (
...
def explore(
    ...
    # Set cache directories to persist on volume (before importing orb_models)
    os.environ["CACHED_PATH_CACHE_ROOT"] = "/data/.cache/cached_path"
    os.environ["HF_HOME"] = "/data/.cache/huggingface"
    os.environ["TORCH_HOME"] = "/data/.cache/torch"

    # Create cache dirs if needed
    os.makedirs("/data/.cache/cached_path", exist_ok=True)
    os.makedirs("/data/.cache/huggingface", exist_ok=True)
    os.makedirs("/data/.cache/torch", exist_ok=True)
    ...
    logger.info(f"Model cache: /data/.cache")
...
def upload_local_db(local_db_path: str | Path = None):
    ...
    Usage:
        from scripts.modal_run import upload_local_db
...
def download_remote_db(local_db_path: str | Path = None):
    ...
    Usage:
        from scripts.modal_run import download_remote_db
...
def run_explore(system: str, **kwargs) -> dict:
    ...
    Usage:
        from scripts.modal_run import run_explore
